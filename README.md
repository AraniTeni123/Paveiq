...
`data/raw/` and `data/processed/` are gitignored — they're pipeline working directories,
regenerated by running the stages above. That means a fresh deploy (e.g. Streamlit Cloud)
has nothing in `data/processed/` for the dashboard to read.

`data/demo/` is a deliberate exception: it holds one committed file,
`*_features_with_wards_scored.parquet` — the final scored output of a real run of the
full pipeline through stage 5, for the Koramangala bbox (`12.93 77.55 12.99 77.65`). The
dashboard's data loader (`dashboard/data.py`) prefers a real `data/processed/*_scored.parquet`
and only falls back to `data/demo/` when one isn't found, so local dev running the full
pipeline is unaffected — the deployed app just has something to show.
